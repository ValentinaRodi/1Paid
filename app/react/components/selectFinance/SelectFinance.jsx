    ...
    return (
        <div ref={selectRef}>
            <div className="popup custom-select">
                <div onClick={openModal} className="flex justify-between items-center pointer font-secondary-med text-[#C5CFE4] text-sm">{valueSelect}
                    <span><img className={rotate} src='/img/icon-arrow-select-fin.svg' alt='arrow'/></span>
                </div>
                {modalOpen ?
                    <div className="select-items_fin">
                        {props.arr.map(item => (
                            <div
                                key={uuid()}
                                className="font-secondary-med text-sm px-3 py-2 text-[#C5CFE4]"

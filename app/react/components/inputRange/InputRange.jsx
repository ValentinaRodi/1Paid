...
const InputRange = ({name, changeFormValue, min, max, styleIcon, styleDiv, reset, changeStateRangeFalse, idInput}) => {
    ...
    useLayoutEffect(() => {
        ...
        if (+minVal !== +min || +maxVal !== +max) {
            ...
            switch (name) {
                case 'Цена':  // if (x === 'value1')
                    name = 'price';
                    break;
                case 'Ранг':  // if (x === 'value1')
                    name = 'rank';
                    break;
                case 'Количество доната':  // if (x === 'value1')
                    name = 'donation_size';
                    break;
            ...
            <div className="range mt-0 min-[880px]:mt-4 mb-4 sm:mb-6 relative">
                ...
                <div className='flex justify-between items-center mb-1 sm:mb-3 pt-6'>
                    <div ref={inputMinRef} className='flex justify-between items-center gap-[5px]'>
                        ...
                        <input name="minVal" type="text"
                               value={(reset === true) ? min.toLocaleString('ru-RU') : minValInput.toLocaleString('ru-RU')}
                               onClick={clearInputMin} onChange={changeMinValue}
                               placeholder={min.toLocaleString('ru-RU')}
                               className='flex items-center h-[33px] w-[85px] rounded-[4px] font-secondary-bold text-bold bg-[#EAEBF8] border-solid border-[1px] border-[rgb(206,208,232,0.25)] text-sm px-2'/>
                    </div>
                    <div ref={inputMaxRef} className='flex justify-between items-center gap-[5px]'>
                        ...
                        <input name="maxVal" type="text"
                               value={(reset === true) ? max.toLocaleString('ru-RU') : maxValInput.toLocaleString('ru-RU')}
                               onClick={clearInputMax} onChange={changeMaxValue}

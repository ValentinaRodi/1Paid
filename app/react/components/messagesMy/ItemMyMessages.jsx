...
function ItemMessagesMy(props) {
    ...
    return (                          
        <div className={`${props.read ? 'bg-[#F3F5FA]' : null} fm-message flex items-center py-3 pr-3 pl-1.5 cursor-pointer hover:bg-[#C8D5ED]/30 border-b border-solid border-[#EAEEF4]`}>
            ...
            <div className="fm-message-user flex flex-col mr-[12px]">
                <div className="fm-message-username font-secondary-bold text-black text-base">{props.nameUser}</div>
                <div className="fm-message-text font-primary-med text-sm">{props.message}</div>
            </div>
            <div className="fm-info py-1 flex flex-col items-end justify-between h-full">
                <span className="fm-message-time font-secondary-med text-xs">{props.time}</span>

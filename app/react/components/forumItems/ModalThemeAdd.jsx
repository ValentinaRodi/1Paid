...
function ModalThemeAdd(props) {
    ...
    useEffect(() => {
        (text.length !== 0 && title.length !== 0) ? setBgColor(true) : setBgColor(false);
    }, [text, title]);
            ...
            <div className='shadow-[0px_25px_35px_0px_#E2E3F3A6] w-full sm:w-[443px] bg-white rounded-lg px-6 pb-6'>
                <div className='w-full flex justify-center'>
                    <div className='flex justify-center items-center px-6 h-[22px] rounded-b-[20px] btn-secondary bg-[linear-gradient(90deg,#8CD23C_0%,#417A00_100%)]'>
                        <p className='family-bold font-bold text-xs text-white uppercase'>Создание темы</p>
                    </div>
                </div>
                ...
                <div className='flex flex-col gap-6'>
                    <textarea id='titleTheme' value={title} onChange={handleInputChangeTitle} className="overflow-hidden h-12 sfb-form-input rounded border border-solid border-[#E3E4F4] w-full px-4 py-3.5 bg-[#EAEBF8] font-secondary-bold text-[11px] md:text-[13px] text-black resize-none duration-200 placeholder:text-[#BAC9E5]" placeholder="Заголовок темы"></textarea>
                    <textarea id='petition' value={text} onChange={handleInputChange} className="overflow-hidden h-32 sfb-form-input rounded border border-solid border-[#E3E4F4] w-full px-4 py-3.5 bg-[#EAEBF8] font-secondary-bold text-[11px] md:text-[13px] text-black resize-none duration-200 placeholder:text-[#BAC9E5]" placeholder="Описание темы"></textarea>
                    <div className='flex justify-between gap-1 items-center'>
                        <button onClick={addBalance} className={`${bgColor ? 'shadow-[0px_4px_24px_0px_#A63DFD40] p-btn cursor-pointer' : 'bg-[#ABABAB] cursor-default'} px-0 text-base rounded-lg w-full h-[60px] text-white `}>Создать тему</button>
                    </div>
                </div>
            </div>
        </div>
